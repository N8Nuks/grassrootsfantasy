...
export default function FieldingClient() {
  ...
  /* Everything static, cut once to its own canvas. Perspective helpers are
     recomputed here rather than shared, so the cache doesn't depend on draw()
     having run first. */
  ...
  return (
    <>
      <style>{`
        ...
        /* Twelve levels as a grid rather than a list — the shared GameLadder is
           built for the vertical form, and forcing it would make it worse. */
      ...
      <GameHud stats={[
        { label: 'Level', value: level, colour: 'var(--neon)' },
        { label: 'Down', value: `${done}/${targetsFor(level)}` },
