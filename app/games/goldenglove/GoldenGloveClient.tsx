...
export default function GoldenGloveClient() {
  ...
  return (
    <>
      ...
      <GameHud stats={[
        { label: 'Level', value: L.name, colour: 'var(--neon)', small: true },
        { label: 'Ball', value: `${Math.min(ballNo, L.balls)}/${L.balls}` },
        { label: 'Clean', value: clean },
        { label: 'Rate', value: ballNo ? `${pct}%` : '—' },
      ]} />
      ...
      <GameLadder
        ...
        rungs={LEVELS.map(lv => ({
          name: lv.name,
          note: `${lv.balls} balls · ${lv.need === 1 ? 'perfect' : '90%'}${lv.plain.length === KINDS.length ? ' · white light' : lv.plain.length > 0 ? ' · some white' : ''}`,
        }))}
      />
    </>
  )
