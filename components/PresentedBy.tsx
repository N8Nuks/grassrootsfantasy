  ...
  return (
    <a href="/nfs/sponsors" className="flex items-center gap-2.5 transition-opacity hover:opacity-100"
      ...
      <span className="rounded flex items-center justify-center shrink-0"
        style={{ width: '42px', height: '42px', background: p.tile ?? '#0D0D0F', border: '1px solid #ffffff18' }}>
        {p.logo && (
          // eslint-disable-next-line @next/next/no-img-element
          <img src={p.logo} alt="" style={{ maxWidth: '76%', maxHeight: '76%', width: 'auto', height: 'auto' }} />
        )}
      </span>
      <span className="text-[11px] font-black uppercase tracking-[0.18em]" style={{ color: '#F5F1E866', lineHeight: 1.45 }}>
        {award}
        <span className="text-[13px]" style={{ display: 'block', color: p.accent, marginTop: '1px' }}>{p.name}</span>
      </span>
    </a>
  )
}