...
const BADGE_LABELS: Record<string, string> = {
  veteran: 'Veteran', club_legend: 'Club Legend', icon: 'Icon',
}
const prettyBadge = (b: string) => BADGE_LABELS[b.toLowerCase()] ?? b.replace(/_/g, ' ')
  ...
  return (
    <div className="w-full" style={{ aspectRatio: '5 / 7.2', maxHeight: '78vh', margin: '0 auto', perspective: '1100px', cursor: flippable ? 'pointer' : 'default' }}
      ...
      <div className="relative w-full h-full"
        style={{ transformStyle: 'preserve-3d', transition: 'transform 0.6s cubic-bezier(0.3, 0.1, 0.3, 1)', transform: flipped ? 'rotateY(180deg)' : 'none' }}>
        ...
        <div className="absolute inset-0 rounded-2xl flex flex-col"
          ...
          <div className="flex-1 rounded-xl overflow-hidden flex flex-col min-h-0"
            ...
            <div className="relative flex items-end justify-center overflow-hidden" style={{ flex: '1 1 auto', minHeight: 0, background: T.surface }}>
              ...
              {/* Badges — longevity and honours pills */}
                ...
                <div className="absolute left-3 flex flex-col gap-1 items-start" style={{ bottom: '10px' }}>
                  {(player.badges ?? []).map(b => (
                    <span key={b} className="text-[8px] font-black uppercase tracking-widest rounded-full"
                      style={{ color: '#141210', background: meta.accent, padding: '3px 9px', boxShadow: `0 0 10px ${meta.accent}70` }}>
                      {prettyBadge(b)}
                    </span>
                  ))}
                </div>
              ...
              {flippable && (
                <span className="absolute bottom-2 right-3 text-[8px] font-bold uppercase tracking-widest"
                  style={{ color: T.textDim, textShadow: '0 0 6px #000000' }}>Tap for round stats ⟳</span>
              )}
            </div>
            {/* Positions row */}
        ...
        <div className="absolute inset-0 rounded-2xl flex flex-col"
          ...
          <div className="flex-1 rounded-xl overflow-hidden flex flex-col min-h-0"
            ...
            <div className="text-center" style={{ background: T.headerBg, borderTop: `1px solid ${meta.accent}40`, padding: '10px 16px' }}>
              <p className="text-[9px] font-bold uppercase tracking-[0.25em]" style={{ color: T.textDim }}>Tap to flip back</p>
            </div>
          </div>
        </div>
      </div>
